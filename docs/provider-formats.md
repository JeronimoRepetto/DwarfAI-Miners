...
- `statusUpdatedAt` dates the `status` write specifically, and the REPL is what writes `status` — so on a session with no transcript yet it is the only stamp that says when a console last spoke, which is what `sitsAtAnOpenPrompt` weighs there (#313). Measured 2026-09-09: the entry landed 97s before the first prompt created the transcript. `startedAt` stands in for a build that reports a status and no stamp for it. **[V]**
...
Two names in that last row earn a line elsewhere even though they earn none here. `request_user_input`
carries the questions the model is waiting on — right to omit from a feed, wrong to omit from the
panel — and `update_plan` carries the agent's own checklist. Both are measured in
`codex-v2-format.md` §9 (issue #265).
...
- **Correction, 2026-09-12 (#264), to item 2 above.** "Rollout mtime recent **and** a `codex.exe` exists" has not been the rule since the SQLite registry landed, and the order matters: growth since the previous scan, then a `logs_2.sqlite` heartbeat for the thread, then the registry row's own stamps, and only then the mtime. The heartbeat is reached **through** the registry row, so the row query's retention floor was silently taking the heartbeat with it — a session logging a line a moment ago was refused as though nothing had been heard from it. See `codex-v2-format.md` §11 for the reading.

- **Second correction, 2026-09-16 (#264), measured live.** The two facts §11 left open are answered in `codex-v2-format.md` §12: `codex resume` **re-stamps** `updated_at_ms` and `recency_at_ms` and **reuses** the thread's `rollout_path`, so a resumed session is fresh on its own terms. Two things in item 2 above are now wrong in the letter. First, the mtime is written **once at open and once at close and never in between** — so the file with the truthful mtime is the CLOSED one, which is the inversion §12(b) records. Second, a headless `codex exec` run writes **no `logs_2.sqlite` rows at all** (§12(c)), so the heartbeat leg is evidence of life and never evidence of its absence, and the `logs.process_uuid` pid join of §8 does not exist for that session shape.
...
Every flag documented in the validated plan's claim-by-claim check is still there on 1.1.26, unchanged since the observer slice's own capture. `buildAntigravityLaunchArgs()` is `['--input-format', 'text']` — `--input-format text` is passed explicitly rather than left to the documented default, for the same reason Claude's and Codex's argv do: a default can move under us. No `--output-format` is passed, because nothing here ever reads the launched process's stdout (`launchRunner.ts`'s `stdio` is `['pipe', 'ignore', 'ignore']` for every provider, unconditionally). **This slice's own stdin assumption — that a bare `-p` with no positional prompt reads its prompt from stdin — was taken on faith from the validated plan's claim ("It can send one prompt through stdin") and turned out to be wrong.** `-p` does not merely fail to help here: it TAKES A VALUE, so pairing it with `--input-format` broke every detached launch this app made. Proven live, on a same-day hotfix (#237), against this same CLI 1.1.26, 2026-09-07: `agy -p --input-format stream-json` exits 2 with `-p took "--input-format" as its prompt`, and a trailing bare `-p` exits 2 with `flag needs an argument: -p`. `--input-format` alone enables print mode without it — `echo "…" | agy --input-format text` printed its reply and exited 0 — so `-p` is dropped from the argv entirely; the stdin write itself (`runLaunchProcess`) is unchanged and fully provider-agnostic.
...
| Codex writes no approval record at all                                                        | Verified for approvals (193 rollouts / 557 MB, 2026-09-09); **user input is the exception** — see `codex-v2-format.md` §9 |
| Codex rollout layout & record types                                                           | Verified on 2 files (0.149.0 TUI + 0.150-alpha Desktop); function_call variant inferred                                   |
| Codex liveness = mtime + task_started/complete + process                                      | **Superseded** — growth, then heartbeat, then registry stamps, then mtime; see `codex-v2-format.md` §11-§12               |
| Codex: `resume` re-stamps `updated_at_ms`/`recency_at_ms` and reuses `rollout_path`           | Verified live on 0.153.4, 2026-09-16 (one thread, read before and after a real resume; §12)                               |
| Codex: a rollout's mtime is written at open and at close, never in between                    | Verified live on 0.153.4, 2026-09-16 (108 KiB written under a frozen mtime, stamped at exit; §12)                         |
| Codex: a headless `codex exec` run writes no `logs_2.sqlite` rows                             | Verified absence on 0.153.4, 2026-09-16 (three runs, row count unchanged, empty WAL; §12)                                 |
| Codex: the relaunch-after-close scenario of #264 still reproduces                             | **Refuted** on 0.153.4, 2026-09-16 — the new session was on the board in the first scan that saw it (§12)                 |
| Codex hooks name the thread and their parent chain names the serving process                  | Verified live on 0.153.4, 2026-09-16 (6 firings / 2 runs; trust-gated, shell-hosted, fires on resume; §13)                |
