...
> **Status (2026-09-09): the last layer this document was waiting on has shipped.** The renderer
> question surface exists — `src/renderer/src/composables/useDwarfQuestion.ts`,
> `components/dwarf/DwarfQuestionCard.vue`, and its sibling `DwarfPermissionCard.vue` for Claude
> Code's permission prompts (#203) — so a held session's question is now asked, drawn above the
> message panel's composer, and answered from a button. Everything below that describes the loop as
> unfinished is a record of how it stood on 2026-09-03, not of how it works. The paragraph after
> this one, and the "what is left" list in §8, are the two places that say so most loudly.
>
> **What was true then, and what this document was for.** For a session the panel itself launches and
> holds, an agent's structured question already reached main live and an answer already reached the
> agent, over a real IPC round trip — nothing here was theoretical. What was missing was the
...
| **External interactive Claude session** (observed, a human at a console) — foreman | **In code, yes; in practice, effectively never while still open**                                                                                                                                               | `parseClaudeTranscriptTail` reading an `AskUserQuestion` `tool_use` block (`src/main/providers/claude/parse.ts:543-610`)                                                                                                                                                                                          | **No structural path.** `dwarf:sendText` can physically put text into that console — by clipboard paste since #319, not by typing it out (`src/main/textDelivery/windowsTextDelivery.ts`, `pasteToConsole`; the character-by-character path in `sendKeys.ts` now serves only #203's single measured permission digit) — but nothing here knows what shape of input the picker expects or maps a `DwarfQuestionOption.label` to a keystroke, and that is the half that has not changed. See §3. | `terminal` — mechanically present, semantically blind                                                                                                                                                                                                                                                                                |
...
1. **A renderer question surface for held sessions.** **SHIPPED — see the status banner.** It went
   where this item guessed it would not: not onto the icon bar or the speech bubble (the two
   components named here as likely hosts, `DwarfActionBar.vue` and `SpeechBubble.vue`, no longer
   exist — the UI rebuild replaced both), but as `DwarfQuestionCard.vue` directly above the message
   panel's composer, with `DwarfPermissionCard.vue` beside it for Claude Code's permission prompts.
   The composable prediction held exactly: `useDwarfQuestion.ts` is a sibling of
   `useDwarfMessaging.ts` and wires `agent:answerQuestion` the same way. The rest of this item is
   kept as written, because it is the reasoning the shipped shape was chosen against. A
