...
The six tracks in this directory are the **maintainer's own**, produced for DwarfAI-Miners (#174).
...
a random order with no repeat until all six have played, then a fresh order, forever. Nothing globs
...
3. Update the count the playlist tests assert, and say in the commit body what the track is.

`.ogg` is what Electron's bundled Chromium decodes natively, so nothing here is transcoded at build
time. Each file costs its own size in the installer; the six together are about 17.6 MB, which the
