...
- **`main/`** — `index.ts` is the composition root, and the only file that owns Electron's
  ...
  simulated one), `runtime` (the poll loop), `sessionLaunch` (starting a session, the opposite act
  to textDelivery), `shell` (window, tray, autostart, shortcuts), `textDelivery`, `tier`.
