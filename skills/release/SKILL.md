...
7. **Packaging never self-publishes.** All three package scripts pass a never-publish flag, and the
   ...
   failed on a missing token. The Linux target also requires an author email AND a in
   : the deb (fpm) target refuses to build without either, while the AppImage builds
   fine, so a Windows-only check never sees the gap (found packaging v0.8.0 by hand).
