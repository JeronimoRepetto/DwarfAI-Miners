...
- **Parent session** `ses_…T2ik`: `parent_id: null`, `agent: "sample-agent"`, `model {"id":"qwen3.6-plus","providerID":"opencode-go"}`, one `part` of `type: "tool"` with **`tool: "task"`**, `state.status: "completed"`, `state.input` present. 34 events (max `seq` 33).
