...
| Write a message into a session's console       | **Verified — the default**                                | Terminal.app: written into the session's tab (on by default); other hosts relay                                                                                                                     | **Unsupported** (relay instead)                                 |
| Relay a message to a named session             | Supported — the fallback                                  | Supported — the fallback for a host other than Terminal.app                                                                                                                                         | Supported — the default                                         |
...
| Answer a permission or question at the console | **Verified**                                              | Terminal.app: permission and single-select answers written into the session's tab; multi-select and "Other Thing" answers take keystrokes (Accessibility, window in front); other hosts unsupported | **Unsupported** — the card sends you to the terminal instead    |
| Attach files to a message                      | **Verified** — console or held Claude session             | Terminal.app console or held Claude session; other hosts held only                                                                                                                                  | Held Claude session only — no console attach control            |
